...
class Bilibili:
    ...
    def login(self, **kwargs):
                    ...
                    if response and response.get("code") is not None:
                        if response["code"] == -105:
                            ...
                            response = self._requests(
                                "get", url, headers=headers, decode_level=1
                            )
                            ...
                            self.access_token = response["data"]["token_info"][
                                "access_token"
                            ]
                            self.refresh_token = response["data"]["token_info"][
                                "refresh_token"
                            ]
                            self._log("登录成功")
                            return True
        ...
        if (
                (not force_refresh_token or not self.access_token or not self.refresh_token)
                and all(
            key in self.get_cookies()
            for key in [
                "bili_jct",
    ...
    def danmaku_post(self, aid, message, page=1, moment=-1):
        ...
        while True:
            payload = {
                ...
                "msg": message,
                "aid": aid,
                "progress": int(moment * 1e3)
                if moment != -1
                else random.randint(0, duration * 1e3),
                "color": 16777215,
                "fontsize": 25,
                "pool": 0,
                "mode": 1,
        ...
        while True:
            ...
            if response and response.get("code") is not None:
                if response["code"] == 0:
                    ...
                    response = self._requests(
                        "get", response["data"]["url"], headers=headers, decode_level=1
                    )
                    captcha = self._solve_captcha(response)
                    if captcha:
    ...
    def dynamic_like(self, did):
        # did = 动态ID
        url = (
            f"{self.protocol}://api.vc.bilibili.com/dynamic_like/v1/dynamic_like/thumb"
        )
        payload = {
            "uid": self.get_uid(),
            "dynamic_id": did,
    ...
    def dynamic_purge(self):
                    ...
                    if response["data"]["has_more"]:
                        dynamics.extend(
                            [
                                ...
                                if card["desc"]["orig_type"] == 2
                                   or card["desc"]["orig_type"] == 1024
                            ]
                        )
        ...
        for dynamic in dynamics:
            ...
            if response and response.get("code") == 0:
                expired = (
                        response["data"]["status"] == 2 or response["data"]["status"] == -1
                )
                winning = any(
                    self.get_uid() in winners
                    for winners in [
                        response["data"]
                            .get("lottery_result", {})
                            .get(f"{level}_prize_result", [])
    ...
    def system_notice(self, time_span=["", ""], keyword=[]):
        ...
        # keyword = 包含关键字
        cursor_span = [
            int(time.mktime(time.strptime(element, "%Y-%m-%d %H:%M:%S")) * 1e9)
            if element
            else ""
            for element in time_span
        ]
        headers = {
        ...
        while True:
            ...
            if response and response.get("code") == 0:
                for notice in response["data"]:
                    if not cursor_span[0] or notice["cursor"] > cursor_span[0]:
                        if not keyword or any(
                                keyword in notice["title"] or keyword in notice["content"]
                ...
                for notice in notice_list:
                    self._log(
                        f"{notice['title']}({notice['time']}): {notice['content']}"
                    )
    ...
    def mall_rush(self, item_id, thread=1, headless=True, timeout=10):
        ...
        def executor(thread_id):
            ...
            driver.get(
                f"{self.protocol}://mall.bilibili.com/detail.html?itemsId={item_id}"
            )
            ...
            while True:
                ...
                if (
                        ...
                        and response["data"]["activityInfoVO"]["serverTime"]
                        >= response["data"]["activityInfoVO"]["startTime"]
                        if response["data"]["activityInfoVO"]
                        else True
                ):
                    break
            timestamp = time.time()
            ...
            while True:
                try:
                    result = {
                        class_name: find_and_click(class_name)
                        for class_name in [
                            "bottom-buy-button",
                            "button",
                            "dot",
                    ...
                    if result["bottom-buy-button"]:
                        if "bottom-buy-disable" not in result[
                            "bottom-buy-button"
                        ].get_attribute("class"):
                            if not in_stock:
                    ...
                    if time.time() - timestamp > timeout:
                        self._log(
                            f"(线程{thread_id})商品{item_id}操作超时, 当前页面为{driver.current_url}"
                        )
                        driver.get(
                            f"{self.protocol}://mall.bilibili.com/detail.html?itemsId={item_id}"
                        )
    ...
    def mall_coupon(self, coupon_id, thread=1):
            ...
            while not flag:
                ...
                if response and response.get("code") is not None:
                    if response["code"] == 83094004:
                        self._log(
                            f"(线程{thread_id})会员购优惠卷\"{coupon_info['name']}\"(ID={coupon_id})领取成功"
                        )
        ...
        if coupon_info:
            if coupon_info["message"] == "可领取":
                ...
                self._log(
                    f"会员购优惠卷\"{coupon_info['name']}\"(ID={coupon_id}){coupon_info['message']}"
                )
                return
        else:
    ...
    def mall_order_list(self, status=0, type=[2]):
        ...
        for order in get_order_list(status, type):
            ...
            order_express = (
                get_order_express(order["order_id"])
                if order_detail and order_detail["orderExpress"]
                else {}
            )
            order_list.append(
                {
                    ...
                    "create": time.strftime(
                        "%Y-%m-%d %H:%M:%S", time.localtime(order.get("order_ctime"))
                    )
                    if order.get("current_timestamp")
                    else None,
                    "status": {
                    ...
                    "pay": {
                        "id": order_detail["orderBasic"].get("payId")
                        if order_detail.get("orderBasic")
                        else None,
                        "time": order.get("pay_ctime")
                        if order.get("pay_ctime") != "0000-00-00 00:00:00"
                        else None,
                        "channel": order_detail["orderBasic"].get("paymentChannel")
                        if order_detail.get("orderBasic")
                        else None,
                        "total": order.get("show_money") / 100
                        if order.get("show_money")
                        else None,
                        "origin": order_detail["orderBasic"].get("payTotalMoney")
                        if order_detail.get("orderBasic")
                        else None,
                        "discount": order_detail["orderBasic"].get("discountMoneys")
                        if order_detail.get("orderBasic")
                        else None,
                        "express": order.get("express_fee") / 100
                        if order.get("express_fee")
                        else None,
                    },
                    "preorder": {
                        "phone": order_detail["extData"].get("notifyPhoneOrigin")
                        if order_detail.get("extData")
                        else None,
                        "front": {
                            "total": order_detail["extData"].get("frontPayMoney")
                            if order_detail.get("extData")
                            else None,
                            "origin": order_detail["extData"].get("frontMoney")
                            if order_detail.get("extData")
                            else None,
                            "discount": order_detail["extData"].get("frontDisMoney")
                            if order_detail.get("extData")
                            else None,
                        },
                        "final": {
                            "total": order_detail["extData"].get("finalPayMoney")
                            if order_detail.get("extData")
                            else None,
                            "origin": order_detail["extData"].get("finalMoney")
                            if order_detail.get("extData")
                            else None,
                            "discount": order_detail["extData"].get("finalDisMoney")
                            if order_detail.get("extData")
                            else None,
                            "start": time.strftime(
                                "%Y-%m-%d %H:%M:%S",
                                time.localtime(
                                    order_detail["extData"].get("finalMoneyStart") / 1e3
                                ),
                            )
                            if order_detail.get("extData")
                               and order_detail["extData"].get("finalMoneyStart")
                            else None,
                            "end": time.strftime(
                                "%Y-%m-%d %H:%M:%S",
                                time.localtime(
                                    order_detail["extData"].get("finalMoneyEnd") / 1e3
                                ),
                            )
                            if order_detail.get("extData")
                               and order_detail["extData"].get("finalMoneyStart")
                    ...
                    "shipping": {
                        "name": order_detail["orderDeliver"].get("deliverName")
                        if order_detail.get("orderDeliver")
                        else None,
                        "phone": order_detail["orderDeliver"].get("deliverPhone")
                        if order_detail.get("orderDeliver")
                        else None,
                        "address": order_detail["orderDeliver"].get("deliverAddr")
                        if order_detail.get("orderDeliver")
                        else None,
                        "company": order_detail["orderExpress"].get("com_v")
                        if order_detail.get("orderExpress")
                        else None,
                        "number": order_detail["orderExpress"].get("sno")
                        if order_detail.get("orderExpress")
                        else None,
        ...
        while True:
            ...
            if response and response.get("code") == 0:
                        ...
                        coupon_list.append(
                            {
                                ...
                                "start": time.strftime(
                                    "%Y-%m-%d %H:%M:%S",
                                    time.localtime(coupon["useStartTime"] / 1e3),
                                )
                                if coupon["useStartTime"]
                                else None,
                                "end": time.strftime(
                                    "%Y-%m-%d %H:%M:%S",
                                    time.localtime(coupon["useEndTime"] / 1e3),
                                )
                                if coupon["useEndTime"]
                                else None,
                                "use": time.strftime(
                                    "%Y-%m-%d %H:%M:%S",
                                    time.localtime(coupon["useTime"] / 1e3),
                                )
                                if coupon["useTime"]
                                else None,
                                "expire": time.strftime(
                                    "%Y-%m-%d %H:%M:%S",
                                    time.localtime(coupon["expireDate"] / 1e3),
                                )
                ...
                for coupon in coupon_list:
                    self._log(
                        f"会员购优惠券: {coupon['name']}"
                        + (
                            f", 失效时间为{coupon['expire']}"
                            if coupon["expire"]
                            else f", 使用时间为{coupon['use']}"
                            if coupon["use"]
                            else f", 使用有效期为{coupon['start']}至{coupon['end']}"
                            if coupon["start"] and coupon["end"]
                            else ""
                        )
                    )
                break
            ...
            if response and response.get("code") == 0:
                        ...
                        prize_list.append(
                            {
                                ...
                                "expire": time.strftime(
                                    "%Y-%m-%d %H:%M:%S",
                                    time.localtime(prize["expireTime"]),
                                ),
                            }
                        )
                if response["data"]["pageInfo"]["hasNextPage"]:
                    ...
                    for prize in prize_list:
                        self._log(
                            f"会员购奖品: {prize['name']}, 来自{prize['source']}, 领取有效期至{prize['expire']}"
                        )
                    break
            else:
    ...
    with open(save_as, "wb") as f:
        ...
        if length:
            ...
            for data in response.iter_content(chunk_size=100 * 1024):
                ...
                print(
                    f"\r[{'=' * int(50 * percent)}{' ' * (50 - int(50 * percent))}] {percent:.0%}",
                    end="",
                    flush=True,
                )
            print()
...
def export(queue, config):
    bucket = {}
    log_file = (
        open(config["global"]["log"], "a", encoding="utf-8")
    ...
    try:
        while True:
            packet = queue.get()
            if isinstance(packet, dict) and all(
                    key in packet for key in ["uid", "manufacturer", "data"]
            ):
                if packet["manufacturer"] == "log":
                    ...
                    if isinstance(packet["data"], list):
                        bucket[packet["manufacturer"]][packet["uid"]].extend(
                            packet["data"]
                        )
                    else:
                        bucket[packet["manufacturer"]][packet["uid"]].append(
                            packet["data"]
                        )
            elif packet is None:
                for manufacturer, data in bucket.items():
                    if config.get(manufacturer, {}).get("export"):
                        with open(
                                config[manufacturer]["export"], "w", encoding="utf-8"
                        ) as f:
...
def wrapper(arg):
        ...
        if isinstance(config["proxy"]["pool"], str):
            try:
                with open(
                        config["proxy"]["pool"],
                        "r",
                        encoding=detect_charset(config["proxy"]["pool"]),
                ) as f:
                    instance.set_proxy(
                        add=[
                            proxy
                            for proxy in f.read().strip().splitlines()
    ...
    if instance.login(
            force_refresh_token=config["user"]["force_refresh_token"], **account
    ):
        threads = []
        ...
        if config["set_privacy"]["enable"]:
            threads.append(
                threading.Thread(
                    target=instance.set_privacy,
                    args=(
                        config["set_privacy"]["show_favourite"],
                        config["set_privacy"]["show_bangumi"],
                        config["set_privacy"]["show_tag"],
                        config["set_privacy"]["show_reward"],
        ...
        if config["watch"]["enable"]:
            threads.append(
                threading.Thread(
                    target=delay_wrapper,
                    args=(instance.watch, 5, list(zip(config["watch"]["aid"]))),
                )
            )
        if config["like"]["enable"]:
            threads.append(
                threading.Thread(
                    target=delay_wrapper,
                    args=(instance.like, 5, list(zip(config["like"]["aid"]))),
                )
            )
            ...
            threads.append(
                threading.Thread(
                    ...
                    args=(
                        instance.reward,
                        5,
                        list(zip(config["reward"]["aid"], config["reward"]["double"])),
                    ),
                )
            )
        if config["favour"]["enable"]:
            threads.append(
                threading.Thread(
                    target=delay_wrapper,
                    args=(instance.favour, 5, list(zip(config["favour"]["aid"]))),
                )
            )
        if config["combo"]["enable"]:
            threads.append(
                threading.Thread(
                    target=delay_wrapper,
                    args=(instance.combo, 5, list(zip(config["combo"]["aid"]))),
                )
            )
        if config["share"]["enable"]:
            threads.append(
                threading.Thread(
                    target=delay_wrapper,
                    args=(instance.share, 5, list(zip(config["share"]["aid"]))),
                )
            )
            ...
            threads.append(
                threading.Thread(
                    ...
                    args=(
                        instance.follow,
                        5,
                        list(zip(config["follow"]["mid"], config["follow"]["secret"])),
                    ),
                )
            )
        if config["follow_batch"]["enable"]:
            threads.append(
                threading.Thread(
                    target=delay_wrapper,
                    args=(
                        instance.follow_batch,
                        5,
                        list(
                            (config["follow_batch"]["mid"][i: i + 50],)
                            for i in range(0, len(config["follow_batch"]["mid"]), 50)
                        ),
                    ),
                )
            )
        if config["ban"]["enable"]:
            threads.append(
                threading.Thread(
                    target=delay_wrapper,
                    args=(instance.ban, 5, list(zip(config["ban"]["mid"]))),
                )
            )
            ...
            threads.append(
                threading.Thread(
                    ...
                    args=(
                        ...
                        5,
                        list(
                            (config["ban_batch"]["mid"][i: i + 50],)
                            for i in range(0, len(config["ban_batch"]["mid"]), 50)
                        ),
                    ),
                )
            )
        if config["danmaku_post"]["enable"]:
            threads.append(
                threading.Thread(
                    target=delay_wrapper,
                    args=(
                        instance.danmaku_post,
                        5,
                        list(
                            zip(
                                config["danmaku_post"]["aid"],
                                config["danmaku_post"]["message"],
                                config["danmaku_post"]["page"],
        ...
        if config["comment_like"]["enable"]:
            threads.append(
                threading.Thread(
                    target=delay_wrapper,
                    args=(
                        instance.comment_like,
                        5,
                        list(
                            zip(
                                config["comment_like"]["otype"],
                                config["comment_like"]["oid"],
                                config["comment_like"]["rpid"],
                            )
                        ),
                    ),
                )
        ...
        if config["comment_post"]["enable"]:
            threads.append(
                threading.Thread(
                    target=delay_wrapper,
                    args=(
                        instance.comment_post,
                        5,
                        list(
                            zip(
                                config["comment_post"]["otype"],
                                config["comment_post"]["oid"],
        ...
        if config["dynamic_like"]["enable"]:
            threads.append(
                threading.Thread(
                    target=delay_wrapper,
                    args=(
                        instance.dynamic_like,
                        5,
                        list(zip(config["dynamic_like"]["did"])),
                    ),
                )
            )
        if config["dynamic_repost"]["enable"]:
            threads.append(
                threading.Thread(
                    target=delay_wrapper,
                    args=(
                        instance.dynamic_repost,
                        5,
                        list(
                            zip(
                                config["dynamic_repost"]["did"],
                                config["dynamic_repost"]["message"],
        ...
        if config["system_notice"]["enable"]:
            threads.append(
                threading.Thread(
                    target=instance.system_notice,
                    args=(
                        config["system_notice"]["time_span"],
                        config["system_notice"]["keyword"],
                    ),
                )
            )
        if config["mall_rush"]["enable"]:
            for item in zip(
                    config["mall_rush"]["item_id"], config["mall_rush"]["thread"]
            ):
                threads.append(
                    threading.Thread(
                        target=instance.mall_rush,
                        args=(
                            item[0],
                            item[1],
                            config["mall_rush"]["headless"],
                            config["mall_rush"]["timeout"],
                        ),
                    )
                )
        if config["mall_coupon"]["enable"]:
            for coupon in zip(
                    config["mall_coupon"]["coupon_id"], config["mall_coupon"]["thread"]
            ):
                threads.append(
                    threading.Thread(
                        target=instance.mall_coupon, args=(coupon[0], coupon[1])
                    )
                )
        if config["mall_order_list"]["enable"]:
            threads.append(
                threading.Thread(
                    target=instance.mall_order_list,
                    args=(
                        config["mall_order_list"]["status"],
                        config["mall_order_list"]["type"],
                    ),
                )
            )
        if config["mall_coupon_list"]["enable"]:
            threads.append(
                threading.Thread(
                    target=instance.mall_coupon_list,
                    args=(config["mall_coupon_list"]["status"],),
                )
            )
        if config["mall_prize_list"]["enable"]:
            threads.append(
                threading.Thread(
                    target=instance.mall_prize_list,
                    args=(
                        config["mall_prize_list"]["status"],
                        config["mall_prize_list"]["type"],
                    ),
                )
            )
    ...
    if config["mall_rush"]["enable"]:
        if platform.system() == "Linux" and os.path.exists("/etc/debian_version"):
            ...
            if shutil.which("chromedriver") is None:
                ...
                os.system(
                    f"{prefix}ln -s /usr/lib/chromium-browser/chromedriver /usr/bin"
                )
            ...
            if not os.path.exists("chrome-win\\chrome.exe"):
                decompress(
                    download(
                        "https://npm.taobao.org/mirrors/chromium-browser-snapshots/Win/706915/chrome-win.zip"
                    )
                )
            if not os.path.exists("chromedriver.exe"):
                decompress(
                    download(
                        "https://npm.taobao.org/mirrors/chromedriver/79.0.3945.36/chromedriver_win32.zip"
                    )
                )
        else:
            ...
            for credential in result:
                ...
                for key, value in credential.items():
                    if value:
                        if key == "cookie":
                            f.write(
                                f"{';'.join(f'{key}={value}' for key, value in value.items())};"
                            )
                        else:
                            f.write(f"{key}={value};")
                        new_line = True
