...
def wrapper(arg):
    ...
    if instance.login(force_refresh_token=config['user']['force_refresh_token'], **account):
        ...
        if config['ban']['enable']:
            threads.append(
                threading.Thread(target=delay_wrapper, args=(instance.ban, 5, list(zip(config['ban']['mid'])))))
        if config['ban_batch']['enable']:
            threads.append(threading.Thread(target=delay_wrapper, args=(instance.ban_batch, 5, list(
                (config['ban_batch']['mid'][i:i + 50],) for i in range(0, len(config['ban_batch']['mid']), 50)))))
